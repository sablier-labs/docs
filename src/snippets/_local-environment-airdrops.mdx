...
PR NOTE: do we really need this tip as airdops package would donwloand the lockup package dep anyway?

:::tip

If you are integrating with `MerkleLL` or `MerkleLT`, you will also have to install
[Lockup npm package](/guides/lockup/examples/local-environment#install-via-npm-package). Both `MerkleLL` and `MerkleLT`
contracts interact with Lockup protocol to setup up vesting for airdropped tokens.
