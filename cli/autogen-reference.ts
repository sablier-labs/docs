...
function run(repo: "airdrops" | "flow" | "lockup") {
  ...
  execSync(`rsync --archive --exclude "README.md" --exclude "SUMMARY.md" repos/${repo}/docs/src/src/* ${contracts}`);
  ...
  execSync(`sd "src/\\w+\\.sol/([\\w.]+)" ${contracts}'/$1' $(find ${contracts} -type f -name "*.md")`);
}
