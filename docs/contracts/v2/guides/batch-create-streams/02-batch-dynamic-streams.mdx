...
[Batch.CreateWithTimestampsLD](/contracts/v2/reference/periphery/types/library.BatchLockup#createwithtimestampsld)
structs:

```solidity
// Declare the first stream in the batch
Batch.CreateWithTimestampsLD memory stream0;
...
```solidity
Batch.CreateWithTimestampsLD memory stream1;
...
Batch.CreateWithTimestampsLD[] memory batch = new Batch.CreateWithTimestampsLD[](batchSize);
batch[0] = stream0;
batch[1] = stream1;
```
