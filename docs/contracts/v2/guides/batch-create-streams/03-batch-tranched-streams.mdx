---
id: "batch-tranched-streams"
sidebar_position: 3
title: "Batch Tranched Streams"
---
...
import { Broker, LockupTranched } from "@sablier/v2-core/src/types/LockupTranched.sol";
...
Create a contract called `BatchLockupTranchedStreamCreator`, and declare a constant `DAI` of type `IERC20`, a constant
`LOCKUP_TRANCHED` of type `ISablierV2LockupTranched`, and a constant `BATCH` of type `ISablierV2Batch`:

```solidity
contract BatchLockupTranchedStreamCreator {
...
- [`createWithDurationsLT`](/contracts/v2/reference/periphery/contract.SablierV2Batch#createwithdurationslt)
- [`createWithTimestampsLT`](/contracts/v2/reference/periphery/contract.SablierV2Batch#createwithtimestampslt)
...
[Batch.CreateWithTimestampsLT](/contracts/v2/reference/periphery/types/library.Batch#createwithtimestampslt) structs:

```solidity
// Declare the first stream in the batch
Batch.CreateWithTimestampsLT memory stream0;
...
```solidity
Batch.CreateWithTimestampsLT memory stream1;
...
Batch.CreateWithTimestampsLT[] memory batch = new Batch.CreateWithTimestampsLT[](batchSize);
batch[0] = stream0;
batch[1] = stream1;
```
...
```solidity
streamIds = BATCH.createWithTimestampsLT(LOCKUP_TRANCHED, DAI, batch);
```
...
[link](https://github.com/sablier-labs/examples/blob/main/v2/periphery/BatchLockupTranchedStreamCreator.sol).

```solidity reference title="Batch Lockup Tranched stream creator"
https://github.com/sablier-labs/examples/blob/main/v2/periphery/BatchLockupTranchedStreamCreator.sol
```
