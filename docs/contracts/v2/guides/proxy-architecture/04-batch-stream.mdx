...
the function, without encoding it in the struct.
...
take maximum advantage of Permit2, we should do a max approval:
...
Once this set-up is complete, we can create a batch stream via the Proxy and Sablier's Proxy Target:
...
Below you can see the complete functioning code: a contract that creates Lockup Linear batch streams via the Proxy and
...
Below you can see the complete functioning code: a contract that creates Lockup Dynamic batch streams via the Proxy and
