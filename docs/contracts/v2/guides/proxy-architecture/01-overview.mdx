...
In the Proxy Target, we use [Permit2](https://github.com/uniswap/permit2#permit2) for all interactions that involve
transferring [ERC-20](https://ethereum.org/en/developers/docs/standards/tokens/erc-20/) tokens from the Proxy owner to
the Proxy contract. To be more specific, we are using the `AllowanceTransfer` contract over the `SignatureTransfer`
