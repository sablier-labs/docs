...
To create a single stream, the caller must approve the creator contract to pull the tokens from the calling address's
...
For the Lockup Dynamic example, we can apply the same steps as for Lockup Linear, we won't detail each step again here.
