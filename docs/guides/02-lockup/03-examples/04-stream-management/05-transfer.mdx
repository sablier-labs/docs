...
that the recipient of the stream has the ability to transfer the NFT to a different address, effectively redirecting
future token releases to that new address.
...
<CodeBlock language="solidity" metastring={`reference title=""`}>
  {`https://github.com/sablier-labs/evm-monorepo/blob/main/misc/examples/lockup/StreamManagement.sol#L84-L89`}
</CodeBlock>
...
<CodeBlock language="solidity" metastring={`reference title=""`}>
  {`https://github.com/sablier-labs/evm-monorepo/blob/main/misc/examples/lockup/StreamManagement.sol#L79-L81`}
</CodeBlock>
