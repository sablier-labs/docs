...
Once the contracts are deployed, the final step is to add the deployment addresses and broadcast files in the docs:
...
2. Copy the latest broadcast files (JSON) from the `broadcast` directory and open a PR in the
