...
The subgraph is designed to track multiple deployments. Therefore, at any given time, there may be multiple instances of
`SablierV2LockupLinear` or `SablierV2LockupDynamic` contracts the indexer will listen to for updates.

A unique `alias` will be attributed to every contract, such that streams will be identifiable through both a long form
and a short form identifier. See the [Stream](#stream) notes below.
...
Based on the schema defined ActionCategory, the following actions will be tracked throughout the subgraph:
...
items at the contract level, we need to consider the following for both subgraphs and our interfaces:
...
Both examples from the table above translate to: **_a stream on Goerli (chain id `5`), within the Lockup contract at
address `0xAB..12`, with the tokenId `21`_**.

:::note

The aliases defined in the subgraph will be used by the app to resolve data about a stream. Make sure to keep them in
sync, avoid conflicts and regard them as immutable (once decided, never change them).

:::

### Aliases

To provide a simple visual structure, while also accounting for future stream types (backwards compatibility) we start
by using following abbreviations as aliases:

- LockupLinear contracts become `LL` (e.g. `LL-5-1`)
- LockupDynamic contracts become `LD` (e.g. `LD-5-1`)
- Contracts with [flash loans](/contracts/v2/guides/flash-loans) enabled become `LLF` or `LLD`

### Relevant parties

Within the larger Sablier V2 ecosystem, the number of relevant entities participating in a stream has grown passed a
`sender` and a `recipient`. Therefore, we identify the following parties involved in a stream.
...
In case of a cancelation, the `sender` will receive any unstreamed funds as part of the refund.
...
[PRBProxy](https://github.com/paulrberg/prb-proxy). The interfaces will mainly provide support for functionality exposed
...
- the `stream.proxender` address (usually an EOA) will control this `sender` proxy and direct it to perform actions on
  its behalf (like a special account)

| #   | Examples of supported flows                                                                                          |
| --- | -------------------------------------------------------------------------------------------------------------------- |
| 1   | `0xF` the funder (EOA), funds a stream on behalf of `0xA`, the sender (EOA), towards `0xB` the recipient             |
