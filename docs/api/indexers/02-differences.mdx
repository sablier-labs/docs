...
In contrast to The Graph, Envio offers a slightly different DX (Developer Experience) as far as indexer implementation.
While for consumers, the query specific don't change a lot, here are a few items to consider before developing on top of
the Sablier indexers/subgraphs:
...
you'll have to identify within the app itself if the query is supposed to yield one or multiple items.

For the 4th example, with Envio the name of an object will yield its `id`, while the `<name>Object` label will ask for
the object itself (e.g. `asset` vs `assetObject`).
