...
If you're looking to support both The Graph and Envio in your apps, see the "Different Results
...
In contrast to The Graph, Envio offers a slightly different developer experience in terms of indexer implementation. While the query syntax isn't that different, there are a few items to consider before developing on top of the Sablier indexers:

### Query Language

Both solutions provide a **GraphQL** API for consumers to read data from. The Graph uses a customized subset of GraphQL
...
- For Envio indexers, entities will have fully specified identifiers (meaning, `chainId` is included)
  1.  `lockup`: the `Action`, `Asset`, `Batch`, `Batcher`, `Contract` have a `-chainId` appended to the ID
  2.  `merkle`: the `Action`, `Asset` and `Factory` have a `-chainId` appended to the ID
