...
If you decide to use both The Graph and Envio, be aware that their GraphQL query syntax differs. Here are some key differences:

| Feature      | The Graph                             | Envio                                      |
| ------------ | ------------------------------------- | ------------------------------------------ |
| Pagination   | `first`, `skip`                       | `limit`, `offset`                          |
| Filter by id | `stream(id)`                          | `Stream_by_pk(id: "...")`                  |
| Multiple     | `streams{}`                           | `Stream(where...){}`                       |
| Nested items | `campaigns{ id, asset: {id, symbol}}` | `Campaign{ asset_id, asset: {id, symbol}}` |

### Notes

- The Graph uses lowercase for single and multiple item queries, while Envio uses capitalized names.
- Envio's indexers provide both the object name and its identifier, which is useful for filtering.
