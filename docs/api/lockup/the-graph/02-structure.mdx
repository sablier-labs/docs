...
A unique `alias` will be attributed to every contract such that streams become identifiable
through either a long form or a short form identifier (e.g. `LK-137-1`). See the [Stream](#stream) section for details.
