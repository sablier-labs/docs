...
We'll break down the [schema](https://github.com/sablier-labs/subgraphs/blob/main/apps/core/schema.graphql) into primary
...
contracts. With Sablier V1.1+, this is not the case any more as streams are now designed without any proxy contracts.
Therefore, you don't need to worry about `proxenders` and `proxy` if you're not looking to support old versions of the
protocol.

:::

[In V2.0] While not mandatory for the core functionality, Sablier made use of [PRBProxy](https://github.com/paulrberg/prb-proxy)
in its architecture during V2.0. The official client interfaces provided support for functionality exposed through both EOAs
and this proxy integration, at the same time.
