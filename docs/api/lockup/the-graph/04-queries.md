...
(e.g. getting the `streamedAmountOf` for each one) make sure you call those methods on the right contract. For indexers,
we store the source contract under `stream.contract.address`.
