...
multi-chain approach (a single endpoint for all chains).

## Configurations

Using this data, we'll run the `pnpm run setup` script. This will do the following:

1. **[Setup]**: Clean artifacts and older files
2. **[Template]**: Convert TS files into JS for mustache to use in the next step
3. **[Template]**: Using mustache, create a specific `config.yaml` (from `config.template.mustache`) using the aggregated
...
To offer a backwards compatible subgraph between the various versions of Sablier Lockup, we'll aggregate
...
`Lockup Tranched` will be bundled under the same `Lockup<Version>` contract tracker in `config.yaml`. Versions of the
protocol will be tracked separately, which is why we have `Lockup_V10` (v1.0) and `Lockup_V11` (12.1) in our
