...
Monthly Unlocks is a special case of Unlock in Steps where assets are unlocked on the same day every month. This is
particularly suited for use cases where users want to distribute tokens every month such as distributing salaries.
...
Each month, on a preset day, a specific amount of tokens becomes unlocked and available for withdrawal. Like the Unlock
in Steps model, unwithdrawn tokens will carry over to the next period, providing flexibility and control to the
recipient. This model is ideal for employers who wish to set up advanced payment schedules for their employees, offering
them access to funds on a predictable, monthly basis.

### Timelock

The Timelock model enables locking tokens for a specified period. This model is designed to foster longer-term
investment and holding strategies by deferring availability.
...
The Unlock Linear model combines an initial immediate release of tokens with a steady, linear payout over time. This
model is ideal for employment contracts that include a signing bonus along with a regular payout schedule.
