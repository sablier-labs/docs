...
At the same time, the model/contract for each shape is not a requirement, but a recommendation - for example, the
Timelock shape can also be implemented via Linear or Dynamic contracts, but is most efficient with the Tranched
contract.
