...
The code used to generate the gas benchmarks for the different stream types can be found
...
These streams are powered by a number of user-provided [segments](/concepts/protocol/segments), which we will cover in
the next article. What is important to note here is that with Lockup Dynamic, Sablier has evolved into a universal
streaming engine, capable of supporting any custom streaming curve.
...
These streams are powered by a number of user-provided [tranches](/concepts/protocol/tranches), which is covered in the
tranches article.
...
Unlock Monthly is a special case of Unlock in Steps where assets are unlocked on the same day every month, e.g. the 1st
