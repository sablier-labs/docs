...
<details>
<summary>Merkle Airdrops v1.3 (Legacy)</summary>

In [Merkle Airdrops v1.3](/guides/airdrops/previous-deployments/v1.3), oracles were not used. As a result, the fees in
v1.3 are specified in the native token. Therefore, the fees in the table above are applicable only to newer versions.
...
When you redeem your tokens from a Bob vault, the fee depends on whether the vault earned yield or not:

- **Vaults that earned yield** (e.g., ETH/WETH vaults with the [Lido adapter](/concepts/bob/lido-adapter)): a percentage
  is taken from the yield you earned. Your original deposit is never touched.
- **Vaults that don't earn yield**: a minimum fee in ETH, same as stream withdrawals.

| Chain    | ETH/WETH vaults    | Other vaults |
| -------- | ------------------ | ------------ |
| Ethereum | 10% from the yield | $0.9 minimum |

### Escrow Trade Fees

Escrow charges a symmetric trade fee when an order is filled i.e. both the seller and buyer pay a percentage of their
respective proceeds.
