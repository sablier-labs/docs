...
A Dynamic stream is composed of multiple segments, which are separate partitions with different token amounts and rates.
The protocol uses these segments to enable custom distribution curves, which power exponential streams, cliff streams,
etc.
...
| :-------- | :-------- | :------------------------------------------------------------------------------------------------ |
| Amount    | `uint128` | The amount of tokens to be distributed in this segment, denoted in units of the token's decimals. |
...
Each segment has its own distribution function:
