...
field of the transaction. During a transaction, [Chainlink oracle](https://chain.link/education/blockchain-oracles) is
used to convert the fee from USD to the native gas token. The transaction succeeds only if the `msg.value` is greater
than or equal to the fee amount returned by `calculateMinFeeWei` function which is available in
...
v1.3 are specified in native token. Therefore, the fees in the table above are applicable only to v2.0 and later. For
