...
The exact value in _Wei_ for a given chain is calculated using
[Chainlink oracles](https://chain.link/education/blockchain-oracles) to obtain the USD price of the gas token at the
...
retroactively. The fee values in individual airdrop contracts remain immutable once set.
...
In the first version of the Merkle Factories (v1.3), oracles were not used to calculate the fee in the gas token, so the
fees are fixed in Wei. Therefore, the [table above](#contract-fees-table) is applicable only to v2.0 and later. For
v1.3, the fees are as follows:
...
| CoreDAO    | CORE      | 0 CORE           | 0                   |
| Gnosis     | XDAI      | 1 XDAI           | 1000000000000000000 |
| HyperEVM   | HYPE      | 0 HYPE           | 0                   |
