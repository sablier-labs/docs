...
import ReleaseHistoryBob from "../../src/snippets/_release-history-bob.mdx";
...
- **The Sablier Protocol**: A collection of persistent, non-upgradeable smart contracts to facilitate ERC-20 token
  distribution over time on Ethereum and other EVM blockchains.
...
- **Flow**: for recurring payments, grants, etc.
...
Sablier is the first project to bring streams to the Ethereum ecosystem, tracing its roots
...
the protocol compared to traditional payment systems, and token distribution over time as an alternative to conventional
payment methods.
...
### Token distribution vs conventional payments
...
By contrast, Sablier enables users to distribute tokens continuously over time through streams. This innovative approach
enables seamless, frictionless transactions and promotes increased financial flexibility for users, businesses, and
other entities. Sablier makes the passage of time itself the trust-binding mechanism, unlocking business opportunities
that were previously unavailable.

A good mental model to contrast Sablier's distribution model with conventional payment models is to view the former as
"real-time finance" or "continuous finance", and the latter as "discrete finance".
